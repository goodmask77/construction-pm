                  ...
                  return (
                    ...
                    >
                      ...
                      {orderedCols.map(col => {
                        ...
                        if (col.id === "name") return <div key={col.id} style={{ ...cs, color: "#211C15", fontWeight: 500 }}><EditableCell catId={catId} itemId={item.id} field="name" value={item.name} /></div>;
  ...
  return (
    <div style={{ paddingTop: 16 }}>
      {[...cats].sort((a,b) => a.order - b.order).map(cat => {
        ...
        return (
          <div key={cat.id} style={{ background: "#ffffff", border: "1px solid #D8CFBB", borderRadius: 12, marginBottom: 10, overflow: "hidden" }}>
            <div onClick={() => setExpanded(e => ({...e, [cat.id]: !e[cat.id]}))} style={{ display: "flex", alignItems: "center", padding: "12px 16px", cursor: "pointer", gap: 12 }}>
              <div style={{ fontSize: 16, color: isExp ? ACCENT : "#6F6656", transition: "transform 0.2s", transform: isExp ? "rotate(90deg)" : "none" }}>▶</div>
              <input
                value={cat.name}
                onChange={e => { e.stopPropagation(); setCats(prev => prev.map(c => c.id === cat.id ? {...c, name: e.target.value} : c)); }}
                onClick={e => e.stopPropagation()}
                style={{ flex: 1, fontWeight: 600, fontSize: 14, color: "#211C15", background: "transparent", border: "none", outline: "none", fontFamily: "'Noto Sans TC', sans-serif", cursor: "text" }}
              />
              <div style={{ fontFamily: "monospace", fontSize: 12, color: ACCENT }}>{fmt(cat.items.reduce((s,it) => s + calcEstimated(it), 0))}</div>
              {catTotal > 0 && <div style={{ fontFamily: "monospace", fontSize: 12, color: catTotal > cat.items.reduce((s,it) => s + calcEstimated(it), 0) ? "#DC2626" : "#3C8C3C" }}>實記 {fmt(catTotal)}</div>}
              <StatusBadge status={cat.status} setCats={setCats} catId={cat.id} />
              <button onMouseDown={e => e.stopPropagation()} onClick={e => { e.stopPropagation(); confirm(`確定刪除「${cat.name}」？`).then(ok => { if (ok) setCats(prev => prev.filter(c => c.id !== cat.id)); }); }} style={{ width: 24, height: 24, borderRadius: "50%", background: "#F3E4DE", border: "1px solid rgba(193,58,34,0.25)", color: "#DC2626", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, cursor: "pointer", flexShrink: 0, padding: 0 }} title="刪除">×</button>
            </div>
            {isExp && (
              <div style={{ borderTop: "1px solid #D8CFBB" }}>
                {/* item table header */}
                <div style={{ display: "grid", gridTemplateColumns: "2fr 60px 60px 70px 70px 80px 80px 100px 80px 30px", gap: 4, padding: "6px 16px", background: "#EFE7D6", fontSize: 10, color: "#6F6656", borderBottom: "1px solid #D8CFBB" }}>
                  <div>項目名稱</div><div>預估數量</div><div>單位</div><div>預估單價</div><div>預估複價</div><div>實際複價</div><div>差異</div><div>負責人</div><div>狀態</div><div />
                </div>
                {cat.items.map(item => (
                  <div key={item.id} onClick={() => onSelectItem(cat, item)} style={{ display: "grid", gridTemplateColumns: "2fr 60px 60px 70px 70px 80px 80px 100px 80px 30px", gap: 4, padding: "8px 16px", borderBottom: "1px solid #D8CFBB44", cursor: "pointer", alignItems: "center", transition: "background 0.15s" }}
                  ...
                  >
                    <input
                      ...
                      onClick={e => e.stopPropagation()}
                      style={{ fontSize: 13, color: item.notes?.includes("⚠️") ? "#C2872E" : "#211C15", background: "transparent", border: "none", outline: "none", fontFamily: "'Noto Sans TC', sans-serif", width: "100%", cursor: "text" }}
                    />
                    <div style={{ fontFamily: "monospace", fontSize: 12, color: "#6F6656" }}>{item.estQty ?? item.qty}</div>
                    <div style={{ fontSize: 11, color: "#6F6656" }}>{item.unit}</div>
                    ...
                    <div style={{ fontFamily: "monospace", fontSize: 12, color: "#3E72A8" }}>{calcActual(item) > 0 ? fmt(calcActual(item)) : "-"}</div>
                    <div style={{ fontFamily: "monospace", fontSize: 12, fontWeight: 600, color: calcActual(item) > calcEstimated(item) ? "#DC2626" : calcActual(item) > 0 ? "#3C8C3C" : "#6F6656" }}>{calcActual(item) > 0 ? (calcActual(item) > calcEstimated(item) ? "+" : "") + fmt(calcActual(item) - calcEstimated(item)) : "-"}</div>
                    <div style={{ fontSize: 12, color: item.assignee ? "#211C15" : "#CDC3AC" }}>{item.assignee || "未指派"}</div>
                    <StatusBadge status={item.status} setCats={setCats} catId={cat.id} itemId={item.id} />
                    <button onMouseDown={e => e.stopPropagation()} onClick={e => { e.stopPropagation(); confirm(`刪除「${item.name}」？`).then(ok => { if (ok) setCats(prev => prev.map(c => c.id === cat.id ? {...c, items: c.items.filter(it => it.id !== item.id)} : c)); }); }} style={{ width: 20, height: 20, borderRadius: "50%", background: "#F3E4DE", border: "1px solid rgba(193,58,34,0.25)", color: "#DC2626", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, cursor: "pointer", padding: 0, flexShrink: 0 }}>×</button>
                  </div>
                ))}
                {/* add item */}
                <div onClick={() => {
                ...
                }} style={{ padding: "8px 16px", color: "#6F6656", fontSize: 12, cursor: "pointer", borderTop: "1px solid #D8CFBB44", display: "flex", alignItems: "center", gap: 6 }}>
      ...
      <div
        ...
        style={{ background: "#FCFAF4", border: "1px dashed rgba(193,58,34,0.3)", borderRadius: 12, padding: "12px 16px", cursor: "pointer", display: "flex", alignItems: "center", gap: 10, color: "#6F6656", fontSize: 13, transition: "border-color 0.2s" }}
        onMouseEnter={e => e.currentTarget.style.borderColor=ACCENT}
        onMouseLeave={e => e.currentTarget.style.borderColor="rgba(193,58,34,0.3)"}
      >
