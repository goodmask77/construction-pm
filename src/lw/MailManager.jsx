...
export default function MailManagerView({ K, canEdit, confirm }) {
  ...
  return (
    <div>
      ...
      <div style={{ fontSize: 11.5, color: C.faint }}>安全機制：刪除＝移到 Gmail 垃圾桶（30 天可救回）；「保留」規則是白名單、永遠不會被動到；處理範圍＝收件匣＋重要郵件＋垃圾郵件夾＋你自建的資料夾；「收件人」規則可清外洩地址（如 privaterelay）的信；內建智慧清理：垃圾夾亂碼信自動刪、收件匣「沒有規則接手＋帶退訂標頭」的訂閱廣告信自動刪（要留的來源請設「保留」或「貼標」規則）；分類好的目標資料夾、寄件備份、垃圾桶不會碰。規則每天自動跑一次。</div>
    </div>
  );
}
